...
namespace DD
{
    public class SpawnHerdIncidentExtension : TimedSpawnExtension
    {
        ...
        
        {
            ...
            defs = defs.Where(def => def != null && def.RaceProps != null && def.RaceProps.meatDef != null && def.RaceProps.meatDef.IsNutritionGivingIngestible && !def.RaceProps.Insect); //Filter out pawns that do not spawn meat.
            defs = defs.Where(def =>
            ...
            defs = defs.Where(def => 
            {
                // Check if Wildness is defined in statBases
                StatModifier wildnessStat = def.race.statBases?.FirstOrDefault(stat => stat.stat == StatDefOf.Wildness);
                return wildnessStat != null ? wildnessStat.value <= maxWildness : false;
            }); // Filter out pawns that have more wildness than the threshold.
            if (debug)
            {
    ...
    public class IncidentWorker_DragonHunting : TrackedIncidentWorker
    {
        ...
        protected override bool TryExecuteWorkerSub(IncidentParms parms)
        {
            ...
            for (int hunter = 0; hunter < hunterCount; hunter++)
            {
                ...
                PawnGenerationRequest req = new PawnGenerationRequest(hunterEntry.kindDef, biologicalAgeRange: new FloatRange(50, 100));
                Pawn hunterPawn = PawnGenerator.GeneratePawn(req);

                //hunterPawn.ageTracker.AgeBiologicalTicks = 180000000;
                //hunterPawn.ageTracker.AgeChronologicalTicks = 180000000;
