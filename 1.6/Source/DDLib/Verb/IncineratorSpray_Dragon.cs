...
public class Verb_ArcSprayIncinerator_Dragon : Verb_ShootBeam
{
    ...
    private IncineratorSpray_Dragon sprayer;

    public override void WarmupComplete()
    {
        sprayer = GenSpawn.Spawn(DD_ThingDefOf.IncineratorSpray_Dragon, caster.Position, caster.Map) as IncineratorSpray_Dragon;
        base.WarmupComplete();
    ...
    protected override bool TryCastShot()
    {
        ...
        MoteDualAttached moteDualAttached = MoteMaker.MakeInteractionOverlay(A: new TargetInfo(caster.Position, caster.Map), moteDef: DD_ThingDefOf.Mote_GraserBeamBase_Dragon, B: new TargetInfo(intVec, caster.Map));
        float num = Vector3.Distance(vector, drawPos);
        float num2 = ((num < BarrelOffset) ? 0.5f : 1f);
        IncineratorSpray_Dragon incineratorSpray_Dragon = sprayer;
        if (incineratorSpray_Dragon != null)
        {
            incineratorSpray_Dragon.Add(new IncineratorProjectileMotion
            {
