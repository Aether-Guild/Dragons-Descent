...
namespace DD
{
    public class Ritual_MaintainNeeds : Ritual_TargetingTicking
    {
        ...
        {
            Messages.Message("RitualControlNeedsOnMessage".Translate(Target.Label.Named("TARGET"), Duration.ToStringTicksToPeriodVague().Named("DURATION")), MessageTypeDefOf.PositiveEvent);
        }

        protected override void PostDeactivation()
        {
            Messages.Message("RitualControlNeedsOffMessage".Translate(Target.Label.Named("TARGET")), MessageTypeDefOf.NegativeEvent);
        }
    }
}
