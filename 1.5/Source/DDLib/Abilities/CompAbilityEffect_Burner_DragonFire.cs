...
public class CompAbilityEffect_Burner_DragonFire : CompAbilityEffect
{
    public new CompProperties_AbilityBurner_DragonFire Props => (CompProperties_AbilityBurner_DragonFire)props;
            ...
            yield return new PreCastAction
            {
                action = delegate (LocalTargetInfo a, LocalTargetInfo _)
                {
                    ...
                    for (int i = 0; i < numStreams; i++)
                    {
                        ...
                        {
                            MoteDualAttached mote = MoteMaker.MakeInteractionOverlay(DD_ThingDefOf.DD_Mote_IncineratorBurst_Fire, new TargetInfo(intVec, map), new TargetInfo(intVec2, map));
