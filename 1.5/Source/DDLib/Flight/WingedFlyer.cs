...
namespace DD
{
    public class WingedFlyer : Thing, IThingHolder
    {
        ...
        public static bool IsCellValid(IntVec3 cell, Map map, bool endpoint)
        {
            ...
            if (endpoint)
            {
                if (cell.Roofed(map) && cell.GetRoof(map) != DD_BiomeRoof.BMT_RockRoofStable)
                {
                    return false;
                }
                //if (cell.Roofed(map))
        ...
        private static List<Pawn> TryCollectRelatedPawns(Pawn pawn, List<Pawn> collection)
        {
            ...
            if (!collection.Contains(pawn))
            {
                ...
                //Type giddyBaseType = Type.GetType("GiddyUp.Base, GiddyUp"); //Base
                //Type giddyStorage = Type.GetType("GiddyUp.Storage.ExtendedDataStorage, GiddyUp"); //ExtendedDataStorage
                //Type giddyPawn = Type.GetType("GiddyUp.Storage.ExtendedPawnData, GiddyUp"); //ExtendedPawnData
